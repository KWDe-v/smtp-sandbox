...
conn
  .on('ready', () => {
    ...
    const command = `
      cd ${REMOTE_DIR} && \
      docker compose exec -T -e SMTP_HOST=smtp backend node dist/scripts/send-test-email.js "seguro_post@asgardcp.com.br" "seguranca@empresa.com" "Seu código de verificação é 998811" "Código: 998811. Válido por 10 minutos."
    `;
