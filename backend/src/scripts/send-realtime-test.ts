...
conn
  .on('ready', () => {
    ...
    const command = `
      cd ${REMOTE_DIR} && \
      docker compose exec -T -e SMTP_HOST=smtp backend node dist/scripts/send-test-email.js "teste@example.test" "notificacoes@empresa.com" "Novo E-mail em Tempo Real" "Mensagem entregue instantaneamente sem recarregar a tela!"
    `;
