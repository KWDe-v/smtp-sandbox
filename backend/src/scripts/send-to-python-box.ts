...
conn
  .on('ready', () => {
    ...
    const command = `
      cd ${REMOTE_DIR} && \
      docker compose exec -T -e SMTP_HOST=smtp backend node dist/scripts/send-test-email.js "teste_python@asgardcp.com.br" "sistema_compras@loja.com" "Seu pedido #98432 foi confirmado!" "Olá! Seu pedido foi confirmado com sucesso via API Sandbox."
    `;
